# AGENTS.md history — dated design/architecture passes (NOT CURRENT)

**This file is dated history, kept for its reasoning. Do not read anything in it as a statement
of current behaviour — check the code, or `INVARIANTS.md` for what still binds.** It is the
maintainer-brief-by-maintainer-brief record of how the UI/architecture reached its present shape:
who reported what, when a ruling reversed, and what a past pass actually changed. Several
sections explicitly describe eras that have since been superseded (a five-tab nav that became
three and then five again; a material system built, replaced, and rebuilt) — later entries in
this same file say so, but nothing here should be trusted over reading the code.

Relocated verbatim from `AGENTS.md`'s "Architecture at a glance" section, session S0.1
(2026-09-01). See `docs/audit/INSTRUCTION_SURFACE_AUDIT.md` for what was verified vs. carried over
...
# Known gotchas — expanded findings (full narrative)

Terse pointers to the load-bearing ones live in `INVARIANTS.md`. This is the full reasoning,
including several that are cross-cutting rather than single-file and so have no one call site to
carry the comment.
...
# Deployment / build history and reasoning (full narrative)

The binding rules (always merge to `main`, which workflow to trigger, bump-before-trigger
sequencing) live in `INVARIANTS.md`. This is the incident history and reasoning behind them —
in particular the Play Store `production`-channel gap found 2026-08-15.
