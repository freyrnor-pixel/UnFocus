...
| `components/ShoppingRow.tsx` (mounted via `components/WeekListCard.tsx`, `app/(tabs)/shopping.tsx` / `app/shopping.tsx`) | `components/ShoppingRow.tsx:314` (`<Animated.View style={[styles.row, ..., { backgroundColor: theme.surface }]}>`), boxed by `components/WeekListCard.tsx:658` (`styles.rowsCard`) | **No.** No `rowList` import in either file. | `components/WeekListCard.tsx:950` — `rowsCard: { borderRadius: Radius.md, paddingHorizontal: Spacing.md, borderLeftWidth: 3 }`, with `borderLeftColor: theme.good` set inline at the call site (`components/WeekListCard.tsx:658`) — a left-border-colored box, not a rail-plus-neutral-fill box. `components/ShoppingRow.tsx:416` — `row: { position: 'relative' }` — no separator constant used between stacked rows at all (`components/WeekListCard.tsx:659`, `<View key={item.id}>` with no hairline). | INSIDE (checkbox/remove controls are drawn as part of `styles.row`'s own content) | **not as-is.** This is the pre-`rowList` left-border-hue-wash shape the module's header describes replacing elsewhere; converging it means dropping `rowsCard`'s left border for a rail and adding the missing separator, i.e. a real behavior change, not an import |
...
- **Components from Part 1 this test does NOT cover:**
  ...
  - `components/ShoppingRow.tsx` / `components/WeekListCard.tsx`
