 ...
 * Phase 3c's NextTaskCard/DayTimeline need so they can typecheck ahead of
 * Phase 5's real task store. Never call in a mounted app — throws to make
 * accidental real usage fail loudly instead of silently no-op'ing.
 *
 * Connections:
 *   Imports → —
 *   Used by → components/QuickAddSheet.tsx, components/NextTaskCard.tsx (Task type + toggle()),
 *             components/DayTimeline.tsx, components/DraggableTaskRow.tsx (Task type only)
 ...
 *     the fields NextTaskCard/DayTimeline read: id/title/time/taskType/
 *     durationMinutes/done/importance) — same minimal-contract precedent as
 *     ShoppingItem in useShoppingStore.ts. `done`/`importance` added for
 *     Phase 3d's DayTimeline (dimming/star indicator).
 */
