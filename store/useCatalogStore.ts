 ...
 *     'seed' rows' price AND category with lib/catalogSeed.ts when the seed version changes, but
 *     never overwrites either once a real purchase sets price_source to 'purchase'.
  ...
  tx(() => {
    for (const s of CATALOG_SEED) {
      ...
      try {
        ...
        // Keep seed-sourced prices AND categories in sync with lib/catalogSeed.ts when the
        // seed version changes. Stops touching the row once a real purchase marks it
        // price_source = 'purchase'.
        db.runSync(
          `UPDATE store_items SET price = ?, category = ?, price_source = 'seed' WHERE id = ? AND price_source = 'seed'`,
          [s.price, s.category, stableId]
        );
      } catch (err) {
