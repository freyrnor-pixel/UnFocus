...
- **Empty-state explainers** (`components/StarterCard.tsx`, 2026-07-26): a second, more visible teaching layer than the ⓘ hint — a short explanation plus one concrete example, rendered inline where content would be while a surface is empty, and gone once the user has their own (gated on a plain `length === 0`, so it also returns if they delete everything). Live on Habits (plus four one-tap starter habits from `lib/habitStarters.ts`), Plans, Shopping, Health, and under the Energy meter (`components/EnergyMeter.tsx`, gated on nothing having an energy value yet). Copy lives under `starters.*` in `lib/i18n.ts`; each one's core message is also in the matching `hints.*.example`, which is where it stays reachable after the card disappears. Styled with a **neutral** `theme.border` Surface, deliberately NOT the accent-barred HintCard look — on a first visit both are on screen at once and twins would read as a duplicate.
