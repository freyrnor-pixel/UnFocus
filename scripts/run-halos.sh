#!/usr/bin/env bash
# Builds the web preview (unless dist/ is already there), serves it, runs the halo audit and
...
node scripts/serve-web.mjs > /tmp/unfocus-halos-serve.log 2>&1 &
SERVER_PID=$!
trap 'kill $SERVER_PID 2>/dev/null' EXIT
sleep 2
node scripts/measure-halos.mjs "$@"
