/**
 * AddDishToMonthlySheet.tsx — pick saved dishes and add them to the Monthly list in place.
 *
 * Centered modal opened from the Monthly tab's "Add dish" trigger. Lists every saved
 * dish grouped by meal type; tapping one pushes its ingredients onto the Monthly list
 * (status:'catalog' rows carrying dishName), so shopping.tsx's groupByDish() renders it
 * as one expandable dish container. The sheet stays open so several dishes can be added
 * for the month's meal plan in one pass — added dishes show a checkmark; "Close" dismisses.
 *
 * This is the in-place counterpart to FoodTab's "Add to monthly list" popup — same push
 * shape, but reachable directly from the Monthly tab without switching to the Food tab.
 *
 * Connections:
 ...
 *   Used by → app/(tabs)/shopping.tsx (Monthly tab "Add dish" trigger)
 *   Data    → none directly — reads useMealStore.dishes, writes via useShoppingStore.add()
 *             (status:'catalog' rows). Calls loadDishes() on open (idempotent; seeds).
 *
 * Edit notes:
 ...
 *     adds it again — useShoppingStore.add() increments matching catalog rows, so a second
 *     tap bumps quantities rather than creating duplicate rows (Decision 021 increment).
 *   - No new i18n keys — reuses addDishSheetTitle / noDishesAvailable / mealTypes /
 *     ingredientsCount / dishAddedToMonthly / closePopupLabel.
 */
...
export default function AddDishToMonthlySheet({ visible, onClose, onAdded }: Props) {
  ...
  // Push a dish's ingredients onto the Monthly list (status:'catalog'), carrying dishName so
  // shopping.tsx's groupByDish() buckets them into one expandable dish container. Mirrors
  // FoodTab.handleAddToMonthly — the same row shape the Monthly tab already groups on.
